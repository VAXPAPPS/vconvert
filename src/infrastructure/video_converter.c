...
static gboolean dispatch_progress(gpointer data) {
    ProgressUpdate *pu = (ProgressUpdate *)data;
    if (pu->task->prog_cb) {
...
static void on_stderr_read(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    GDataInputStream *stream = G_DATA_INPUT_STREAM(source_object);
    VideoTask *task = (VideoTask *)user_data;
    GError *error = NULL;
    gsize length;
    ...
    if (line != NULL) {
        ...
        if (time_str && task->duration > 0.0) {
            ...
            if (sscanf(time_str, "time=%d:%d:%lf", &h, &m, &s) == 3) {
                ...
                g_idle_add(dispatch_progress, pu);
            }
        }
        
        g_free(line);
        g_data_input_stream_read_line_async(stream, G_PRIORITY_DEFAULT, NULL, on_stderr_read, task);
    } else {
...
static void on_video_subprocess_finished(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    ...
    if (g_subprocess_wait_finish(subprocess, res, &error)) {
        ...
        g_printerr("Video conversion failed: %s\n", error->message);
        g_error_free(error);
    }
...
void convert_video_async(const gchar *input_path, const gchar *output_path,
    ...
    VideoTask *task = g_new0(VideoTask, 1);
    task->prog_cb = prog_cb;
    task->fin_cb = fin_cb;
    task->user_data = user_data;
    task->duration = 0.0;
    
    g_print("Starting video conversion: %s -> %s\n", input_path, output_path);
    
    GSubprocess *subprocess = g_subprocess_new(
        G_SUBPROCESS_FLAGS_STDOUT_PIPE | G_SUBPROCESS_FLAGS_STDERR_PIPE,
        &error,
        "ffmpeg", "-y", "-i", input_path, output_path, NULL
    );
    
    if (error != NULL) {
        g_printerr("Failed to spawn ffmpeg for video: %s\n", error->message);
        g_error_free(error);
    ...
    g_data_input_stream_read_line_async(data_stream, G_PRIORITY_DEFAULT, NULL, on_stderr_read, task);
    g_subprocess_wait_async(subprocess, NULL, on_video_subprocess_finished, task);
}
